...
class EventsProvider with ChangeNotifier {
  ...
  Future<List<Event?>> fetchPastEvents() async {
    QuerySnapshot snapshot =
    await FirebaseFirestore.instance.collection('events').get();
    //print("snapshot lollll ${snapshot}");
    List<Event?> pastEvents = snapshot.docs.map((d) {
      //print(d);
      //print(d["eventStartDate"].toDate().compareTo(DateTime.now()));
      if (d["eventEndDate"].toDate().compareTo(DateTime.now()) < 0) {
        print("past event");
        return Event(
            eventId: d["eventId"],
  ...
  Future<List<Event?>> fetchEvents() async {
    ...
    await FirebaseFirestore.instance.collection('events').get();
    //print("snapshot lollll ${snapshot}");
    List<Event?> Events = snapshot.docs.map((d) {
      //print(d);
      //print(d["eventStartDate"].toDate().compareTo(DateTime.now()));
      if (d["eventEndDate"].toDate().compareTo(DateTime.now()) < 0 ||
          d["eventEndDate"].toDate().compareTo(DateTime.now()) >= 0) {
        print("all event");
        return Event(
            eventId: d["eventId"],
            eventDescription: d["eventDescription"],
